...
- Nginx: public ports 80/443; HTTP redirects to HTTPS and HTTPS proxies public
  API requests to the Backend

The Backend reads secrets from `/etc/dsm-p5-g02/backend.env`. This file must be
owned by `root:dsmapp`, have mode `0640`, and define at least `MONGODB_URI` and
the non-empty `SYNC_API_TOKEN`.
...
CORS_ORIGINS=https://<public-client-origin>
TRUSTED_PROXY_ADDRESSES=127.0.0.1,::1,::ffff:127.0.0.1
RATE_LIMIT_GLOBAL_MAX=100
RATE_LIMIT_WINDOW_MS=60000
RATE_LIMIT_ML_PREDICT_MAX=10
RATE_LIMIT_COMPATIBILITY_MAX=10
RATE_LIMIT_SYNC_MAX=5
```

## TLS and Nginx

Provision a certificate whose Subject Alternative Name is the IP address
`158.158.48.119`. Do not use a certificate valid only for a DNS name. The
checked-in Nginx configuration expects:

- certificate chain: `/etc/dsm-p5-g02/tls/fullchain.pem`
- private key: `/etc/dsm-p5-g02/tls/privkey.pem`

These are configuration paths, not tracked certificate files. To use other
paths, change `ssl_certificate` and `ssl_certificate_key` in
`infra/nginx/dsm-p5-g02.conf` before deployment. Keep the private key readable
only by root (for example mode `0600`), then validate and reload:

```bash
sudo nginx -t
sudo systemctl reload nginx
curl --fail https://158.158.48.119/health
```

No health check uses `--insecure`; the machine trust store and the certificate
IP SAN must validate. The public Nginx layer returns 404 for `/api/sync` and
`/api/sync/*` before the generic `/api/` proxy. Administrative synchronization
must be invoked locally against `127.0.0.1:3000` with the Bearer token. Public
responses include HSTS, anti-sniffing, frame, referrer and permissions headers.
...
The final public check resolves `158.158.48.119` to loopback, performs normal TLS
verification, and requires JSON values `status: ok` and `database: connected`.
...
After deployment, CD checks `https://158.158.48.119/health` and fails unless TLS
is valid and the JSON reports both the API and database as ready.
